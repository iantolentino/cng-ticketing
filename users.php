...
<div class="page-head"><div><p class="eyebrow">Super Admin</p><h1>Users</h1><p class="page-subtitle">Create accounts, assign roles, and manage individual permission overrides.</p></div><button class="button" type="button" data-user-drawer-trigger aria-expanded="false" aria-controls="user-create-drawer">New user</button></div>
...
<?php if ($editing): ?><section><h2>Edit user</h2><form method="post" class="form-grid"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="save"><input type="hidden" name="id" value="<?= (int) $editing['id'] ?>"><label>Full name<input name="full_name" required value="<?= e($editing['full_name'] ?? '') ?>"></label><label>Username<input name="username" required pattern="[A-Za-z0-9._-]{3,80}" value="<?= e($editing['username'] ?? '') ?>"></label><label>Email<input name="email" type="email" required value="<?= e($editing['email'] ?? '') ?>"></label><label>Role<select name="role_id" required><?php foreach ($roles as $role): ?><option value="<?= (int) $role['id'] ?>" <?= (int) ($editing['role_id'] ?? 0) === (int) $role['id'] ? 'selected' : '' ?>><?= e($role['name']) ?></option><?php endforeach; ?></select></label><label>Department<select name="department_id"><option value="">None</option><?php foreach ($departments as $department): ?><option value="<?= (int) $department['id'] ?>" <?= (int) ($editing['department_id'] ?? 0) === (int) $department['id'] ? 'selected' : '' ?>><?= e($department['name']) ?></option><?php endforeach; ?></select></label><label class="checkbox-label"><input type="checkbox" name="is_active" <?= $editing['is_active'] ? 'checked' : '' ?>> Active account</label><div><button class="button">Save changes</button></div></form></section><?php endif; ?><aside class="user-create-drawer" id="user-create-drawer" data-user-drawer hidden role="dialog" aria-modal="true" aria-labelledby="user-create-drawer-title"><div class="drawer-header"><div><p class="eyebrow">User management</p><h2 id="user-create-drawer-title">Create user</h2><p class="drawer-description">Add an account and assign its access.</p></div><button class="icon-button" type="button" data-user-drawer-close aria-label="Close new user panel">×</button></div><form method="post" class="user-drawer-form"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="save"><input type="hidden" name="id" value="0"><label>Full name<input name="full_name" required></label><label>Username<input name="username" required pattern="[A-Za-z0-9._-]{3,80}"></label><label>Email<input name="email" type="email" required></label><label>Role<select name="role_id" required><option value="" selected disabled>Select a role</option><?php foreach ($roles as $role): ?><option value="<?= (int) $role['id'] ?>"><?= e($role['name']) ?></option><?php endforeach; ?></select></label><label>Department<select name="department_id"><option value="">None</option><?php foreach ($departments as $department): ?><option value="<?= (int) $department['id'] ?>"><?= e($department['name']) ?></option><?php endforeach; ?></select></label><div class="drawer-footer"><button class="button button-secondary" type="button" data-user-drawer-close>Cancel</button><button class="button" type="submit">Create user</button></div></form></aside><div class="user-drawer-scrim" data-user-drawer-scrim hidden></div>
