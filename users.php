...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'save') {
            ...
            if ($id > 0) {
                $pdo->prepare('UPDATE users SET full_name=?, username=?, email=?, role_id=?, department_id=?, is_active=? WHERE id=?')->execute([$fullName, $username, $email, $roleId, $departmentId, isset($_POST['is_active']) ? 1 : 0, $id]);
                $notice = 'User updated.';
            } else {
