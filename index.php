...
<div class="table-wrap"><table class="ticket-table"><thead><tr><?php foreach (['ID', 'Title', 'Status', 'Priority', 'Assignee', 'SLA', 'Created', 'Last update'] as $heading): ?><th><?= e($heading) ?></th><?php endforeach; ?><th aria-label="Open ticket"></th></tr></thead><tbody><?php foreach ($tickets as $ticket): $sla = ticket_sla_state($ticket); $priority = $ticket['priority'] ?? 'normal'; $statusLabel = $ticket['status_label'] ?? ($statuses[$ticket['status']] ?? $ticket['status']); $priorityLabel = $ticket['priority_label'] ?? (TICKET_PRIORITIES[$priority] ?? ucfirst($priority)); $ticketUrl = !empty($ticket['is_external']) ? 'ticket.php?external=' . rawurlencode($ticket['external_key'] . ':' . $ticket['external_id']) : 'ticket.php?id=' . (int) $ticket['id']; $createdAt = (string) ($ticket['created_at'] ?? ''); $updatedAt = (string) ($ticket['closed_at'] ?? $ticket['updated_at'] ?? $createdAt); ?><tr class="ticket-row-<?= e($sla[0]) ?>" data-ticket-href="<?= e($ticketUrl) ?>" tabindex="0" aria-label="Open <?= e($ticket['ticket_number']) ?>: <?= e($ticket['subject']) ?>"><td class="ticket-id"><a href="<?= e($ticketUrl) ?>"><?= e(ticket_display_id($ticket)) ?></a></td><td class="subject"><a href="<?= e($ticketUrl) ?>"><?= e($ticket['subject']) ?></a><span class="ticket-row-meta"><?= e($ticket['employee_name'] ?? $ticket['requester'] ?? '') ?> · <?= e($ticket['departments'] ?? '') ?><?= !empty($ticket['category']) ? ' · ' . e($ticket['category']) : '' ?><?php if (!empty($ticket['is_external'])): ?> · <span class="source-badge"><?= e($ticket['source']) ?></span><?php endif; ?></span></td><td><span class="pill pill-<?= e(str_replace('_', '-', $ticket['status'])) ?>"><?= e($statusLabel) ?></span></td><td><span class="pill pill-priority-<?= e($priority) ?>"><?= e($priorityLabel) ?></span></td><td class="ticket-assignee"><?= e($ticket['assignees'] ?? $ticket['agent'] ?? 'Unassigned') ?></td><td class="ticket-sla"><span class="sla-badge sla-<?= e($sla[0]) ?>"><?= e($sla[1]) ?></span><small><?= e($sla[2]) ?> · <?= ticket_age_days($ticket, 'created_at') ?>d open</small></td><td class="ticket-date"><?= e($createdAt ? date('M j, Y', strtotime($createdAt)) : '—') ?><small><?= e($createdAt ? date('g:i A', strtotime($createdAt)) : '') ?></small></td><td class="ticket-date"><?= !empty($ticket['closed_at']) ? 'Closed' : 'Updated' ?><small><?= e($updatedAt ? date('M j, Y', strtotime($updatedAt)) : '—') ?></small></td><td class="row-arrow"><a href="<?= e($ticketUrl) ?>" aria-label="Open <?= e($ticket['ticket_number']) ?>">&rsaquo;</a></td></tr><?php endforeach; ?><?php if (!$tickets): ?><tr><td colspan="9" class="muted">No tickets match the selected filters.</td></tr><?php endif; ?></tbody></table></div>
