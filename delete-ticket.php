...
$pdo=db();$pdo->beginTransaction();try{activity($id,$user['id'],'soft_deleted',['ticket_number'=>$ticket['ticket_number']]);$pdo->prepare('UPDATE tickets SET deleted_at=NOW(),deleted_by=? WHERE id=?')->execute([$user['id'],$id]);$pdo->commit();redirect('index.php');}catch(Throwable $e){$pdo->rollBack();http_response_code(500);exit('Ticket could not be deleted.');}
