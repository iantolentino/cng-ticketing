...
<div class="roles-access-screen">
    ...
    <section class="access-panel access-users-panel" aria-labelledby="access-users-title">
        <div class="access-section-head"><div><h2 id="access-users-title">Users</h2></div><span><?= count($users) ?> users</span></div>
        <div class="table-wrap access-users-wrap">
            <table class="ticket-table access-users-table">
                <thead><tr><th scope="col">User</th><th scope="col">Role</th><th scope="col">Status</th><th scope="col">Actions</th></tr></thead>
                <tbody>
                <?php foreach ($users as $member): $formId = 'user-access-form-' . (int) $member['id']; ?>
                    <tr>
                        <td class="access-user-cell" data-label="User"><span class="access-user-avatar" aria-hidden="true"><?= e(strtoupper(substr((string) $member['full_name'], 0, 1))) ?></span><span><strong><?= e($member['full_name']) ?></strong><small>@<?= e($member['username']) ?></small></span></td>
                        <td class="access-role-cell" data-label="Role">
                            <div class="filter-picker access-role-picker" data-filter-picker data-access-form="<?= e($formId) ?>">
                                <input type="hidden" form="<?= e($formId) ?>" name="role_id" value="<?= (int) $member['role_id'] ?>">
                                <button type="button" class="filter-picker-trigger" data-filter-trigger aria-expanded="false" aria-label="Change role for <?= e($member['full_name']) ?>"<?= $canManageUsers ? '' : ' disabled' ?>><span class="filter-picker-copy"><strong data-filter-label><?= e($member['role_name']) ?></strong></span><svg class="filter-picker-chevron" viewBox="0 0 24 24" aria-hidden="true"><path d="m7 9 5 5 5-5"/></svg></button>
                        ...
                        <td class="access-status-cell" data-label="Status"><span class="access-account-state<?= $member['is_active'] ? ' is-active' : ' is-inactive' ?>"><i aria-hidden="true"></i><?= $member['is_active'] ? 'Active' : 'Inactive' ?></span></td>
                        <td class="access-action-cell" data-label="Actions">
                            <form id="<?= e($formId) ?>" method="post" class="user-access-form" data-submit-on-change="false">
                                <input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="user"><input type="hidden" name="user_id" value="<?= (int) $member['id'] ?>">
                                <input type="hidden" value="1" data-access-active-input<?= $member['is_active'] ? ' name="is_active"' : '' ?>>
                                <button type="button" class="access-switch<?= $member['is_active'] ? ' is-on' : '' ?>" data-access-switch aria-pressed="<?= $member['is_active'] ? 'true' : 'false' ?>" aria-label="Toggle account status for <?= e($member['full_name']) ?>"<?= $canManageUsers ? '' : ' disabled' ?>><span class="access-toggle-track" aria-hidden="true"><i></i></span><span data-access-switch-label><?= $member['is_active'] ? 'Enabled' : 'Disabled' ?></span></button>
                                <button type="submit" class="button access-save-button"<?= $canManageUsers ? '' : ' disabled' ?>>Save</button>
                                <?php if ($canManageUsers): ?><a class="access-reset-link" href="reset-user-password.php?id=<?= (int) $member['id'] ?>">Reset</a><?php endif; ?>
                            </form>
                        </td>
                    </tr>
                <?php endforeach; ?>
